...
function System(
    data::PowerSystemTableData,
    database::Union{String, Bool, Nothing};
    time_series_resolution = nothing,
    time_series_in_memory = false,
    time_series_directory = nothing,
    runchecks = true,
    ...
    # build the system using the function already defined above
    System(data; time_series_resolution, time_series_in_memory, time_series_dictionary, runchecks, kwargs...)

    # create SQLite database if kwarg database is true or a string (which will be the name of the database)
    if !isnothing(database)
        if typeof(database) == Bool && istrue(database)
            make_database(sys = sys, database_name = nothing, timeseries_metadata_file = timeseries_metadata_file)
        elseif typeof(database) == String
            make_database(sys = sys, database_name = database, timeseries_metadata_file = timeseries_metadata_file)
        end
    end
end
=#
